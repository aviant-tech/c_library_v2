...
static void mavlink_test_aviant_indicator_mr_thrust(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
#ifdef MAVLINK_STATUS_FLAG_OUT_MAVLINK1
    mavlink_status_t *status = mavlink_get_channel_status(MAVLINK_COMM_0);
        if ((status->flags & MAVLINK_STATUS_FLAG_OUT_MAVLINK1) && MAVLINK_MSG_ID_AVIANT_INDICATOR_MR_THRUST >= 256) {
            return;
        }
#endif
    mavlink_message_t msg;
        uint8_t buffer[MAVLINK_MAX_PACKET_LEN];
        uint16_t i;
    mavlink_aviant_indicator_mr_thrust_t packet_in = {
        17.0,45.0,29
    };
    mavlink_aviant_indicator_mr_thrust_t packet1, packet2;
        memset(&packet1, 0, sizeof(packet1));
        packet1.thrust_margin = packet_in.thrust_margin;
        packet1.thrust_margin_omi = packet_in.thrust_margin_omi;
        packet1.state = packet_in.state;
        ...
        if (status->flags & MAVLINK_STATUS_FLAG_OUT_MAVLINK1) {
           // cope with extensions
           memset(MAVLINK_MSG_ID_AVIANT_INDICATOR_MR_THRUST_MIN_LEN + (char *)&packet1, 0, sizeof(packet1)-MAVLINK_MSG_ID_AVIANT_INDICATOR_MR_THRUST_MIN_LEN);
        }
#endif
        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_aviant_indicator_mr_thrust_encode(system_id, component_id, &msg, &packet1);
    mavlink_msg_aviant_indicator_mr_thrust_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_aviant_indicator_mr_thrust_pack(system_id, component_id, &msg , packet1.state , packet1.thrust_margin , packet1.thrust_margin_omi );
    mavlink_msg_aviant_indicator_mr_thrust_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_aviant_indicator_mr_thrust_pack_chan(system_id, component_id, MAVLINK_COMM_0, &msg , packet1.state , packet1.thrust_margin , packet1.thrust_margin_omi );
    mavlink_msg_aviant_indicator_mr_thrust_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
    ...
    mavlink_msg_aviant_indicator_mr_thrust_decode(last_msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
        
        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_aviant_indicator_mr_thrust_send(MAVLINK_COMM_1 , packet1.state , packet1.thrust_margin , packet1.thrust_margin_omi );
    mavlink_msg_aviant_indicator_mr_thrust_decode(last_msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

#ifdef MAVLINK_HAVE_GET_MESSAGE_INFO
    MAVLINK_ASSERT(mavlink_get_message_info_by_name("AVIANT_INDICATOR_MR_THRUST") != NULL);
    MAVLINK_ASSERT(mavlink_get_message_info_by_id(MAVLINK_MSG_ID_AVIANT_INDICATOR_MR_THRUST) != NULL);
#endif
}
